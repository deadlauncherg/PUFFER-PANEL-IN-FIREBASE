...
# ========== lapiogaming Combined VPS Installer Script ==========
# Cleaned, fixed, and production-ready
# ===============================================================

ROOTFS_DIR=$(pwd)
ARCH=$(uname -m | tr -d '[:space:]')  # ensure no hidden characters
...
# Ensure basic tools
apt update && apt install -y wget curl tar bash gnupg ca-certificates

# Rootfs install if not already done
if [ ! -f "$ROOTFS_DIR/.installed" ]; then
  echo "#################### Installing Ubuntu RootFS ####################"
  read -p "Do you want to install Ubuntu? (YES/no): " confirm
  if [[ "$confirm" =~ ^[Yy][Ee]?[Ss]?$ ]]; then
    wget --tries=50 --timeout=10 -O /tmp/rootfs.tar.gz \
  ...
  wget --tries=50 --timeout=10 -O "$ROOTFS_DIR/ubuntu/usr/local/bin/proot" \
    "https://raw.githubusercontent.com/foxytouxxx/freeroot/main/proot-${ARCH_ALT}"
...
apt update && apt install -y bash curl wget gnupg lsb-release sudo iputils-ping lsof gnupg2 ca-certificates software-properties-common
